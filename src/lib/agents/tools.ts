...
async function callApi(
  ...
  try {
    // Use the provided apiUrl or fall back to the base URL from env
    const base = (apiUrl || API_BASE_URL).replace(/\/$/, "");
    const url = `${base}/hackathon${endpoint}`;
    ...
    const response = await fetch(url, {
      ...
      headers: {
        ...
        "ngrok-skip-browser-warning": "true", // Add this for ngrok support
      },
    ...
    // If data has "success" field, it's already in ToolResponse format
